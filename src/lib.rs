...
///   crl_number: CrlNumber::from(1234),
...
/// A certificate revocation list number.
#[derive(Debug, PartialEq, Eq, Hash, Clone)]
...
impl CrlNumber {
	/// Create a certificate revocation list number from the given byte slice.
	pub fn from_slice(bytes :&[u8]) -> Result<CrlNumber, RcgenError> {
		// RFC 5280 §5.2.3:
		...
		if bytes.len() > 20 {
			return Err(RcgenError::InvalidCrlNumber)
		}
		let inner = bytes.to_vec();
		Ok(CrlNumber { inner })
	}
	/// Return the byte representation of the certificate revocation list number.
	pub fn to_bytes(&self) -> Vec<u8> {
		self.inner.clone()
	}
	/// Return the length of the certificate revocation list number in bytes.
...
impl From<Vec<u8>> for CrlNumber {
	fn from(inner :Vec<u8>) -> CrlNumber {
		CrlNumber { inner }
	}
}

impl From<u64> for CrlNumber {
	fn from(u :u64) -> CrlNumber {
		let inner = u.to_be_bytes().into();
		CrlNumber { inner }
	}
}

impl AsRef<[u8]> for CrlNumber {
	fn as_ref(&self) -> &[u8] {
		&self.inner
	}
}
...
impl RevokedCertParams {
	fn write_der(&self, writer :DERWriter) {
		writer.write_sequence(|writer| {
			...
			// TODO(@cpu): Skip if reason code is unspecified.
			let should_write_exts = self.reason_code.is_some() || self.invalidity_date.is_some();
			if should_write_exts {
				writer.next().write_sequence(|writer| {
...
pub enum RcgenError {
	...
	UnsupportedInCsr,
	/// Invalid certificate revocation list (CRL) number.
	InvalidCrlNumber,
...
impl fmt::Display for RcgenError {
	fn fmt(&self, f :&mut fmt::Formatter) -> fmt::Result {
		...
		match self {
			...
			InvalidCrlNumber => write!(f, "Invalid CRL number parameter")?,
