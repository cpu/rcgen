...
fn test_botan() {
	...
	check_cert(&cert.borrow_der(), &cert);
}
...
fn test_botan_separate_ca() {
	...
	check_cert_ca(&cert.borrow_der(), &cert, &ca_cert.borrow_der());
}
...
fn test_botan_imported_ca() {
	...
	let ca_cert_der = ca_cert.borrow_der();
	...
	let cert = params
		...
		.unwrap();
	check_cert_ca(&cert.borrow_der(), &cert, &ca_cert_der);
}
...
fn test_botan_crl_parse() {
	...
	let botan_ee = botan::Certificate::load(&ee.borrow_der()).unwrap();
