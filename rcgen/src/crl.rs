...
use crate::{CertifiedKey, Error, KeyIdMethod, KeyUsagePurpose, SerialNumber, SignatureAlgorithm};
...
/// let issuer = CertifiedKey::generate_self_signed(issuer_params).unwrap();
...
impl CertificateRevocationList {
	...
	pub fn serialize_der_with_signer(&self, ca: &CertifiedKey) -> Result<Vec<u8>, Error> {
		if !ca.cert.params.key_usages.is_empty()
			&& !ca
				.cert
				.params
				.key_usages
		...
		self.params.serialize_der_with_signer(
			ca.cert.params.alg,
			&ca.key_pair,
			&ca.cert.params.distinguished_name,
		)
	}
	/// Serializes the certificate revocation list (CRL) in ASCII PEM format, signed with
	/// the issuing certificate authority's key.
	#[cfg(feature = "pem")]
	pub fn serialize_pem_with_signer(&self, ca: &CertifiedKey) -> Result<String, Error> {
