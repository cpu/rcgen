...
[`CertifiedKey::generate_self_signed`] function.
*/
#![cfg_attr(
	feature = "pem",
	doc = r##"
...
/// An issued certificate together with its subject keypair.
pub struct CertifiedKey {
	key_pair: KeyPair,
	cert: Certificate,
}

/// An issued certificate and the parameters that describe it.
...
pub fn generate_simple_self_signed(
	...
	CertifiedKey::generate_self_signed(CertificateParams::new(subject_alt_names))
}
...
impl CertificateParams {
	...
	/// [`serialize_der_with_signer`](CertifiedKey::serialize_der_with_signer) or
	/// [`serialize_pem_with_signer`](CertifiedKey::serialize_pem_with_signer)
	/// functions.
	///
...
/// (method three). In each case the first 160 bits of the hash are used as the key identifier
/// to match the output length that would occur were SHA1 used (a legacy option defined in RFC
/// 5280).
///
...
impl CertifiedKey {
	...
	///
	/// If you want to control the certificate subject [`KeyPair`] or the randomness used to
	...
	pub fn generate_self_signed(mut params: CertificateParams) -> Result<Self, Error> {
		...
		let cert_der = params.serialize_der_with_signer(
			&key_pair,
			params.alg,
			&key_pair,
			&params.distinguished_name,
		)?;
		let cert = Certificate { params, cert_der };
	...
	/// generate it, set the [`CertificateParams::key_pair`] field ahead of time before calling
	/// this function.
	pub fn generate(mut params: CertificateParams, issuer: &CertifiedKey) -> Result<Self, Error> {
		let key_pair = Self::prepare_key_pair(&mut params.key_pair, params.alg)?;
		let cert_der = params.serialize_der_with_signer(
			&key_pair,
			&issuer.cert.params.alg,
			&issuer.key_pair,
		...
		yasna::try_construct_der(|writer| {
			writer.write_sequence(|writer| {
				let cert_data = yasna::try_construct_der(|writer| {
					self.cert.params.write_request(&self.key_pair, writer)
				})?;
				writer.next().write_der(&cert_data);

				// Write signatureAlgorithm
				self.cert.params.alg.write_alg_ident(writer.next());
...
#[cfg(feature = "zeroize")]
impl zeroize::Zeroize for CertifiedKey {
	fn zeroize(&mut self) {
		self.cert.params.zeroize();
...
mod tests {
	...
	mod test_pem_serialization {
		...
		fn test_windows_line_endings() {
			let cert = CertifiedKey::generate_self_signed(CertificateParams::default()).unwrap();
