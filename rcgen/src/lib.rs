...
[`Certificate::generate_self_signed`] function.
*/
#![cfg_attr(
	feature = "pem",
	doc = r##"
...
let (cert, key_pair) = generate_simple_self_signed(subject_alt_names).unwrap();
println!("{}", cert.pem());
...
/// An issued certificate together with a subject keypair.
...
) -> Result<(Certificate, KeyPair), Error> {
...
impl CertificateParams {
	...
	/// which you want to sign a certificate newly generated by `rcgen` using
	/// [`Certificate::generate`] or [`Certificate::generate_request`] functions.
	///
...
impl Certificate {
	...
	/// this function.
	pub fn generate_self_signed(mut params: CertificateParams) -> Result<(Self, KeyPair), Error> {
...
mod tests {
	...
	fn test_with_key_usages() {
		...
		// Make the cert
		let cert = Certificate::generate_self_signed(params).unwrap().0;
...
-----END PRIVATE KEY-----"#;
			...
			let ca_cert = Certificate::generate_self_signed(params).unwrap().0;
